...
def open_game_engine():
    script_states.ScriptState.set_script_state(utilities.Enum.ScriptStateType.PRE_ENGINE_OPEN)
    command = utilities.get_unreal_editor_exe_path()
    utilities.run_app(command, utilities.Enum.ExecutionMode.ASYNC, utilities.get_engine_launch_args())
    script_states.ScriptState.set_script_state(utilities.Enum.ScriptStateType.POST_ENGINE_OPEN)


def close_game_engine():
    script_states.ScriptState.set_script_state(utilities.Enum.ScriptStateType.PRE_ENGINE_CLOSE)
    if utilities.get_win_dir_type() == utilities.Enum.PackagingDirType.WINDOWS_NO_EDITOR:
    ...
    script_states.ScriptState.set_script_state(utilities.Enum.ScriptStateType.POST_ENGINE_CLOSE)
